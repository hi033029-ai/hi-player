...
class FileManagerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val fileManagerPrefs = application.getSharedPreferences("file_manager_preferences", Context.MODE_PRIVATE)

    private val _currentDirectory = MutableStateFlow<File>(rootDir)
    val currentDirectory = _currentDirectory.asStateFlow()

    val preferredFileTab: String
        get() = fileManagerPrefs.getString("preferred_file_tab", "FOLDERS_BROWSER") ?: "FOLDERS_BROWSER"

    fun setPreferredFileTab(tab: String) {
        fileManagerPrefs.edit().putString("preferred_file_tab", tab).apply()
    }
    ...
    private val _archiveAwaitingDestination = MutableStateFlow<FileItem?>(null)
    val archiveAwaitingDestination = _archiveAwaitingDestination.asStateFlow()

    private val _extractionDestinationDirectory = MutableStateFlow<File?>(null)
    val extractionDestinationDirectory = _extractionDestinationDirectory.asStateFlow()

    private val _extractionDestinationFolders = MutableStateFlow<List<FileItem>>(emptyList())
    val extractionDestinationFolders = _extractionDestinationFolders.asStateFlow()
    ...
    fun beginExtractTo(item: FileItem) {
        _archiveAwaitingDestination.value = item
        setExtractionDestination(rootDir)
    }

    private fun setExtractionDestination(directory: File) {
        if (!directory.exists() || !directory.isDirectory || !directory.canRead()) return
        _extractionDestinationDirectory.value = directory
        _extractionDestinationFolders.value = try {
            directory.listFiles()
                ...
                ?.map { folder ->
                    FileItem(
                        ...
                        isDirectory = true,
                        childCount = folder.listFiles()?.size ?: 0,
    ...
    fun chooseExtractionDestination(folder: FileItem) {
        if (folder.isDirectory) setExtractionDestination(folder.file)
    }

    fun navigateExtractionDestinationUp() {
        val current = _extractionDestinationDirectory.value ?: return
        val parent = current.parentFile
        if (parent != null && parent.canRead() && current.absolutePath != rootDir.absolutePath) {
            setExtractionDestination(parent)
        }
    }

    fun cancelExtractTo() {
        _archiveAwaitingDestination.value = null
        _extractionDestinationDirectory.value = null
        _extractionDestinationFolders.value = emptyList()
    }

    fun confirmExtractTo(context: Context) {
        val archive = _archiveAwaitingDestination.value ?: return
        val destination = _extractionDestinationDirectory.value ?: return
        cancelExtractTo()
        extractArchiveInternal(archive, context, targetDir = destination)
    }
    ...
    fun installApk(item: FileItem, context: Context) {
        if (!item.isApk || !item.file.exists()) {
            Toast.makeText(context, "APK file is unavailable", Toast.LENGTH_SHORT).show()
            return
        }
        try {
            ...
            Toast.makeText(context, "No package installer found for ${item.name}", Toast.LENGTH_SHORT).show()
        }
    }
